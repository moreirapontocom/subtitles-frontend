...
const VideosList = () => {
    ...
    function getVideos() {
        ...
        return list.map(video => {
            return <tr key={video.url}>
                <td>{video.id}</td>
                <td><span className="tag status">{video.status}</span></td>
                <td><img src={video.cover} alt={video.titulo} height="100" /></td>
                <td><Link to={`/videos/${video.id}`}>{video.titulo}</Link></td>
